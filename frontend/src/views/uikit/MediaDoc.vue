...
const galleriaResponsiveOptions = ref<ResponsiveOption[]>([
...
const carouselResponsiveOptions = ref<ResponsiveOption[]>([
...
onMounted(() => {
    ...
    PhotoService.getImages().then((data: Photo[]) => {
        images.value = data;
    });
});

function getSeverity(status: Product['inventoryStatus']): 'success' | 'warning' | 'danger' | null {
    switch (status) {
        case 'INSTOCK':
            return 'success';
        case 'LOWSTOCK':
