...
const autoValue = ref<Country[] | null>(null);
const selectedAutoValue = ref<Country[] | null>(null);
...
const listboxValues = ref<Country[]>([
...
const listboxValue = ref<Country | null>(null);

const dropdownValues = ref<Country[]>([
    { name: 'New York', code: 'NY' },
    { name: 'Rome', code: 'RM' },
    { name: 'London', code: 'LDN' },
    { name: 'Istanbul', code: 'IST' },
...
const dropdownValue = ref<Country | null>(null);

const multiselectValues = ref<Country[]>([
...
const multiselectValue = ref<Country[] | null>(null);
...
const treeSelectNodes = ref<TreeNode[] | null>(null);
...
function searchCountry(event: { query: string }) {
    setTimeout(() => {
        if (!event.query.trim().length) {
            ...
            autoFilteredValue.value = (autoValue.value || []).filter((country) => country.name.toLowerCase().startsWith(event.query.toLowerCase()));
        }
    }, 250);
}
</script>
